...
{
    private readonly IDictionary<ReflectedHttpActionDescriptor, string[]> actionParams =
        ...
        HttpControllerContext controllerContext, IEnumerable<ReflectedHttpActionDescriptor> actionsFound)
    {
        ...
        if (hasRouteParameters || hasQueryParameters)
        {
            ...
            actionsFound =
                actionsFound.Where(descriptor => this.actionParams[descriptor].All(combinedParameterNames.Contains));
            if (actionsFound.Count() > 1)
            {
                actionsFound =
                    actionsFound.GroupBy(descriptor => this.actionParams[descriptor].Length)
        ...
        {
            actionsFound = actionsFound.Where(descriptor => this.actionParams[descriptor].Length == 0);
        }
        return actionsFound;
    }
